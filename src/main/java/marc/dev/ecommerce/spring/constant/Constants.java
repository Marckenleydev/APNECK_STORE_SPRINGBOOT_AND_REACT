...
public class Constants {
    ...
    public static final String CUSTOMER_AUTHORITIES = "document:read";
    ...
    public static final String MANAGER_AUTHORITIES = "user:create,,user:update,document:create,document:read,document:update,document:delete";
    ...
    public static final  String SELECT_DOCUMENTS_BY_NAME_QUERY = "SELECT doc.id, doc.document_id, doc.name, doc.description, doc.uri, doc.icon, doc.size, doc.formatted_size, doc.extension, doc.reference_id, doc.created_at, doc.updated_at, CONCAT(owner.first_name, ' ', owner.last_name) AS owner_name, owner.email AS owner_email, owner.phone AS owner_phone, owner.last_login AS owner_last_login, CONCAT(updater.first_name, ' ', updater.last_name) AS updater_name FROM documents doc JOIN users owner ON owner.id = doc.created_by JOIN users updater ON updater.id = doc.updated_by WHERE doc.name LIKE CONCAT('%', :documentName, '%')";
    public static final String SELECT_COUNT_DOCUMENTS_BY_NAME_QUERY = "SELECT COUNT(*) FROM documents WHERE name LIKE CONCAT('%', :documentName, '%')";
}
